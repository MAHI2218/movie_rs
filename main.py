...
if not TMDB_API_KEY:
    ...
    raise RuntimeError(
        "TMDB_API_KEY missing. Put it in .env as TMDB_API_KEY=e040ed734b2e5792c597ef141e07b9cb"
    )
...
def load_pickles():
    ...
    # Some dataset exports store the TF-IDF vectorizer in df.pkl. Rebuild the
    # title-only frame needed by the recommendation ranking from indices.pkl.
    if not isinstance(df, pd.DataFrame):
        if hasattr(indices_obj, "items"):
            df = pd.DataFrame(
                {"title": [title for title, _ in sorted(
                    indices_obj.items(), key=lambda item: int(item[1])
                )]}
            )
        else:
            raise RuntimeError(
                "df.pkl must contain a DataFrame or indices.pkl must be mapping-like"
            )
    ...
    # Load the vectorizer when available; recommendations use the saved matrix.
